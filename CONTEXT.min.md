...
- Local `.env` files apuntan al proyecto Supabase **Dev** (`iimqxacagxuemwgaunis`, us-west-1); VPS apunta a Prod (`gmrlhhszrdahcxyoywvt`). Dev tiene keepalive weekly ping via GitHub Actions `.github/workflows/keepalive-supabase-dev.yml` (cron cada 5 días) para evitar auto-pause del free-tier
- Backups de Prod: Supabase Free no tiene backups → dump nocturno (roles/esquema/datos) cifrado con age en el repo PRIVADO `xacty/webbrief-backups` (artifacts, retención 30 días; `webbrief` es público y no sirve para esto). Clave privada solo del owner. Restauración y simulacro (siempre en Dev, nunca en Prod) en `docs/WEBRIEF_OPERATIONS_GUIDE.md` → "Backups de la base de Prod"
...
- `target=editor.updates-panel`
  - `keep`: grouped section activity (one row per section, expandable "Ver detalle" history), compact deliverables, share-link action, internal scroll, bottom-docked content-rules card for document projects; click activity scrolls to section with yellow flash; only section_edited + asset_uploaded events here; orden de grupos SIEMPRE por posición: `__seo__` → `__document__` → secciones del doc en su posición → desconocidas al final por evento más viejo (`lib/activityOrdering.js`; el nivel intermedio que había, secciones solo-en-propuesta, desapareció con el sistema de propuestas — 2026-09-11); marcar leída una fila NUNCA la mueve (solo apaga dot/bold); "Actividad general" queda solo para uploads huérfanos (sin sectionId o de otra página)
- `target=editor.navbar`
...
- `target=editor.collab`
  ...
  - `watch`: `serverPagesRef` es la base del merge — se refresca en load/save-success/sync (el 4to trigger que tenía, proposal-decision, desapareció con el sistema de propuestas — ver `target=editor.proposals` removido 2026-09-11); sessionId por pestaña (`crypto.randomUUID`); secciones remote-origin se agregan a `protectedEmptySectionIds` antes del splice (auto-remove); TipTap v3 `setContent` emite update por default → hidratación usa `{ emitUpdate: false }` o genera autosaves fantasma; tipo `document` resuelve conflictos `__document__` vía banner sobre el canvas + comparador sin "Insertar debajo" (setContent doc completo, historial normal); panel FAQ muestra dots de presencia/conflicto (mismos props que SectionsPanel); dropdowns portal (KebabMenu/PageIndexMenu/PagePill) comparten `frontend/src/hooks/useAnchoredDropdown.js` (modo controlado para PagePill via openMenuId; focus-return al trigger al cerrar); el comparador (4to sink `dangerouslySetInnerHTML`) YA está sanitizado: pasa por `sanitizeContentHtml()` (`frontend/src/lib/sanitizeHtml.js`, DOMPurify) igual que Preview/Handoff/SharePage — cerrado en la auditoría 2026-08 (v2.15.3, commit `cbe9219`); es defensa en profundidad sobre el saneo del backend, no quitarlo al editar el componente
- `target=editor.deeplinks`
  - `keep`: formato `?p=<pageId>&s=<sectionId>` (query params, NO path segments) en `/project/:id/editor` y `/share/:token`; `p` se sincroniza con la página activa vía `setSearchParams(..., { replace: true })` — nunca push, no ensucia el historial; `s` SOLO se consume al cargar (nunca se sincroniza en vivo: pelearía con el scroll-listener del editor) y se genera con acciones explícitas de copiado; params inválidos o ausentes → fallback silencioso a la primera página / sin scroll; el token del share no cambia de formato, los params se appendean a la URL copiada
...
- `target=projects.org`
  - `keep`: carpetas de proyectos son SOLO organización — borrarlas JAMÁS toca proyectos (reparent a padre/raíz, sin papelera de carpetas); barra `MorphingToolbar` (idle: búsqueda/tipo/orden/toggle grid-lista; con selección muta sin reflow); chips de carpetas + `?folderId=`; DnD `application/x-webrief-projects` con toast Deshacer 15s SOLO para mover-a-carpeta; grid conserva selección por checkbox (cards con botones internos), la vista lista usa click-selecciona; WeBrief interna sin menú/drag/checkbox; componentes compartidos en `components/organizer/` (ActionToast, ItemContextMenu items-planos, MorphingToolbar, dnd.js, sorting.js) — biblioteca y proyectos importan de ahí, NO duplicar
  - `watch`: `resolveCompanyRole` en `lib/folderTree.js` incluye membresía 'admin' (bug F1-T5 corregido en 8620d7f — portar a feat/image-library si se mergea sola); orden del DELETE reparent (subcarpetas→projects→row) es OBLIGATORIO por las FKs cascade/set-null; caches `webrief:company:<id>` invalidados tras mover-a-carpeta y CRUD de carpetas; bulk endpoints preexistentes intactos
- `target=library`
  - `keep`: ingesta SIEMPRE convierte (foto→WebP q80 ≤2560, PNG lossless resize, GIF/SVG passthrough) y NUNCA persiste el original; cuota por empresa cuenta activos+papelera con "Vaciar papelera" como válvula; auto-trash post-export solo vía cliente tras descarga confirmada y NUNCA toca assets referenciados en páginas (partition `kept` con reason `referenced`); carpetas: ciclos rechazados, papelera en cascada con timestamp de correlación (restore solo revive miembros de SU cascada); rutas bulk/* declaradas antes de rutas con :param; multiselección solo assets (carpetas via kebab singular); picker del editor inserta por URL pública con attrs completos (assetId/fileName/storagePath); galería con tabs `Biblioteca | Documentos | Briefs` (query param `tab=`): Documentos y Briefs son SOLO-lectura + export agrupados por proyecto — sin subida, carpetas, mover, renombrar ni papelera (esos archivos viven en documentos o son material recibido); las categorías son EXACTAMENTE las de `summarizeUsage` (brief = `uploaded_by IS NULL`; documentos = `project_id NOT NULL` + `uploaded_by NOT NULL`; biblioteca = `project_id IS NULL`), así que tabs y barra de almacenamiento nunca discrepan
  - `watch`: `resolveLibraryRole` lee camelCase de req.currentUser; export sin rate limiter (espejo de export-bulk); tope 100 ids en bulk/export (un grupo con >100 imágenes hace fallar "Descargar todas" con el 400 del backend — no se trunca en silencio); el input file del toolbar del editor vive duplicado cuando hay overflow — usar label+input propio, jamás ref compartida; `buildLibraryListing` expone `allFolders` para el árbol del MoveToFolderModal; `GET /library?section=documents|briefs` ignora `folderId`/`view`, suma `page_id`/`section_id`/`uploaded_by` al select y devuelve `projects: [{id,name}]` (mapa mínimo para agrupar sin fetch por grupo); "Ir al documento" arma el deep-link `?p&s` de `target=editor.deeplinks` y nunca se oculta por falta de `page_id`; el kebab de `AssetGrid` se arma con los callbacks realmente pasados (por eso el tab Biblioteca queda idéntico) y `Exportar` sólo aparece en imágenes — los no-imagen (PDF/Office de brief) usan `Descargar` directo a `public_url` porque el ZIP pasa por transformaciones de ImageKit; `isImageAsset` acepta `mime_type` legacy `"image"` a secas y cae a width/height
...
### Session 18 (2026-05-19) — Dev↔Prod schema sync + advisor WARN cleanup

- **Audit cruzado Dev↔Prod**: verificado vía MCP que las 22 tablas + 4 funciones compartidas + 3 buckets storage son byte-identical en columnas/tipos/constraints. Particularmente las 9 tablas v1.1-relevantes (`profiles`, `companies`, `company_memberships`, `notifications`, `security_events`, `application_errors`, `password_reset_requests`, `rate_limit_buckets`, `security_blocks`) ✓ 92 cols totales idénticas.
- **Drift #1 — `project_page_change_proposals`** (existía sólo en Dev, faltaba en Prod): la usa `backend/src/routes/projects.js` para designer-proposal flow → endpoint en Prod fallaba si se llamaba. Aplicada en Prod migration `20260518xxxxxx_sync_project_page_change_proposals` replicando la DDL exacta de Dev (PK + 4 FKs cascade/SET NULL + CHECK constraint en status + RLS habilitado, sin policies, sin triggers).
- **Drift #2 — `brief_responses.respondent_email`/`.respondent_name`** (Dev: nullable, Prod: NOT NULL): aplicado en Dev `ALTER TABLE … SET NOT NULL`. Safe porque el endpoint `POST /api/public/brief/:token/submit` (public.js:376) valida ambos campos presentes antes del insert. Dev tenía 0 rows al aplicar.
- **Drift #3 — `rls_auto_enable()` + event trigger `ensure_rls`** (existían sólo en Prod): event trigger DDL que auto-habilitaba RLS en cualquier nueva tabla. Dropeado de Prod para alinear con Dev (el otro agente lo había quitado en el split para cerrar advisor WARN). Las migrations futuras siguen incluyendo `ENABLE ROW LEVEL SECURITY` explícito, así que el safety net no se pierde.
- **Bonus: 2 advisor WARNs `function_search_path_mutable`** cerrados en ambos DBs aplicando `ALTER FUNCTION … SET search_path = public, pg_temp` a `set_updated_at()` y `schedule_project_lifecycle_notifications(uuid,text,timestamptz)`.
- **Verificación advisor post-cleanup**: solo queda 1 WARN (`auth_leaked_password_protection` — requiere Supabase Pro, deferred). Los 22 INFOs sobre "RLS enabled, no policy" son intencionales por design (backend usa service_role; anon/authenticated no acceden directo).
- Todas las migrations sync registradas en `supabase_migrations.schema_migrations` con MCP `apply_migration`. Sin commits al repo — son operaciones de schema sync, no features.
...
- **Plan MCP de WeBrief**: `docs/WEBRIEF_MCP_PLAN.md` revisado y mejorado: 4 brechas críticas (auth identity por stdio, edición granular sin endpoints PATCH, invariantes solo en frontend, contrato `expectedVersion` sin snapshot) + 8 mejoras menores; agregadas secciones `Politica De Fetch De URLs` y `Autenticacion MCP Local`; tools renombradas `documents.*` → `pages.*`. Strategy A locked para v1. MCP server NO implementado aún.
- **Handoff para próxima sesión MCP**: `docs/WEBRIEF_MCP_HANDOFF.md` — orden de lectura, decisiones lockeadas, sequencing de Sessions N+1 a N+4 (Prep A token + Prep B invariantes + Fase 0 scaffolding + Fases 1-3), scaffolding sugerido para `mcp/webrief-server/`, tokens estimados (~380-580k v1 completo). Leer al inicio de la próxima sesión MCP.
...
- ~~**Pre-existing advisor WARNs**: `function_search_path_mutable` en `set_updated_at` y `schedule_project_lifecycle_notifications`; `anon/authenticated_security_definer_function_executable` en `rls_auto_enable()`~~ ✓ los 3 cerrados en sesión 18 (drop trigger + función para rls_auto_enable, search_path pinned para los otros 2). Verificado vía `get_advisors`.
