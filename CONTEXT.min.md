...
- Local `.env` files apuntan al proyecto Supabase **Dev** (`iimqxacagxuemwgaunis`, us-west-1); VPS apunta a Prod (`gmrlhhszrdahcxyoywvt`). Dev tiene keepalive via GitHub Actions `.github/workflows/keepalive-supabase-dev.yml` (cron `17 */6 * * *`, 4 runs/día × 3 consultas) para evitar auto-pause del free-tier; la cadencia de 5 días no alcanzaba y Dev se pausó el 2026-08-31
