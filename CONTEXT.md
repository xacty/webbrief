...
- Dev keepalive: GitHub Actions workflow `.github/workflows/keepalive-supabase-dev.yml` (cron `17 */6 * * *` + `workflow_dispatch`) hace 3 GET al REST API de Dev (`companies`, `projects`, `project_pages`) con la anon key, 4 veces al día = 12 requests diarias a la base. Secrets del repo `xacty/webbrief`: `SUPABASE_DEV_URL`, `SUPABASE_DEV_ANON_KEY`. Trigger manual: `gh workflow run keepalive-supabase-dev.yml`.
- Criterio real del auto-pause (no es "7 días sin requests"): Supabase pausa proyectos free que no reciben *suficiente* actividad de base en la ventana de 7 días — la doc pide "a few user requests to the database each day over the previous week" (https://supabase.com/docs/guides/platform/free-project-pausing). La versión original del cron (1 ping cada 5 días) devolvía HTTP 200 y aun así Dev se pausó el 2026-08-31; restaurado el 2026-09-01. Al pausar, Supabase elimina el registro DNS del proyecto, por eso el síntoma es `curl: (6) Could not resolve host`, no un error HTTP.
- El workflow falla explícitamente si ninguna de las 3 consultas devuelve 2xx: un 401 por anon key rotada saldría "verde" con la lógica vieja (aceptaba cualquier no-5xx) sin generar actividad real.
...
## Completed (2026-05-08) — Session 10b: comments UX polish + right-click menu + history extendido

Iteración intensa sobre el sistema de comments inicial: rediseño total de UI a estilo Google Docs en margen, right-click context menu, fixes profundos de selection-preservation, auto-resolución de huérfanos y extensión del History tab a `document`/`faq`.

**Cards de comments en margen derecho (replazan tab del UpdatesPanel)**: la tab `Comentarios` competía por espacio con Activity/Historial y se cortaba. Eliminada. Reemplazo: `frontend/src/components/editor/CommentMarginCards.jsx` con cards flotantes `position: absolute` ancladas al span `[data-comment-id]` en el editor. Layout pass top-down con greedy push-down resuelve overlaps; si la card activa requiere más arriba, todo el stack se shifta respetando gap de 12px. MutationObserver al `editor.view.dom` re-mide en cada edición. ResizeObserver + listeners de scroll/resize repositionan en vivo.

**Cards en dos estados (idle/active)**: idle muestra solo CommentEntry (avatar + autor + tiempo + body + badge "N respuestas" si hay replies). Active expandido con replies completas + ReplyComposer. Sin reply input visible cuando idle — interfaz limpia. Resolver/reabrir y ⋮ están en el header del root (trailing del CommentEntry), no overlapping.

**Menú ⋮ por comentario** (`CommentMenu` interno): items condicionales según permisos: `Editar` (autor + ventana 15min), `Eliminar` (autor o admin), `Copiar link al comentario`. Aparece on-hover (opacity 0→1 con CSS `:hover`) para no clutter visual. Handler `handleCopyCommentLink` en ProjectEditor: `${origin}/project/${id}/editor?commentId=${rootId}` copiado vía `navigator.clipboard.writeText`. Apertura desde URL: useEffect en mount busca `commentId` en query params, switchea de página si hace falta, llama `handleSelectThread`, limpia el param via `history.replaceState`.

**`@menciones` con keyboard navigation completa**: extracción a `frontend/src/components/editor/MentionsAutocomplete.jsx` con helpers compartidos exportados (`detectMentionQuery`, `filterMembers`, `insertMention`, `filterMentionsByBody`). Reusables entre `CommentComposerPopover` (para nuevos comments) y `ReplyComposer` (para replies en margin cards y inline popover). Filter requiere `query.length >= 1` (no se abre dropdown con `@` solo). Keyboard nav estándar: `ArrowDown`/`ArrowUp` mueven `selectedIndex` (clamp), `Enter`/`Tab` insertan el item activo, `Escape` cierra. Click sigue funcionando. Reset del índice cuando cambia la query. CSS `.mentionItemActive` (#e8f0fe) distinto de `:hover` (#f0f4f9) para distinguir keyboard-focus.

**Render de menciones como mailto links** (estilo Google Docs): `renderMentionedBody({body, mentions, profilesById})` parsea el body buscando `@FullName` y solo cuando matchea con un perfil presente en `comment.mentions[]` (validación server-side) lo reemplaza con `<a href="mailto:email" class="wb-mentionLink">@FullName</a>`. Estilo: azul `#1a73e8`, hover underline. `@palabra` casual queda como texto plano. Click en link hace `stopPropagation` para no activar el card.

**Right-click context menu** (`frontend/src/components/editor/EditorContextMenu.jsx`): listener `contextmenu` a nivel `document` en EditorPanel suprime el menú nativo del browser DENTRO del editor (preventDefault). Items: Cortar/Copiar/Pegar/Pegar sin formato/Eliminar — Comentar/Insertar enlace — Negrita/Itálica/Subrayado/Tachado — Tipo de bloque (submenu P/H1-H6)/Limpiar formato. Cortar/Copiar/Pegar usan `navigator.clipboard` con fallback a `document.execCommand`. Defiere al `TableRightClickMenu` cuando el click es en una tabla (check `e.target.closest('table')`). `useLayoutEffect` mide el menú renderizado y, si no hay espacio abajo, lo flippea hacia arriba (anchor bottom al click point); si tampoco entra arriba, pin al borde con más espacio. `visibility: hidden` en el primer paint mientras se mide para evitar flash.

**Preservar selección en right-click** (Google Docs–style, fix complejo): el browser limpia la DOM selection en contenteditable al hacer right-click; ProseMirror sincroniza desde `selectionchange` event y la pierde aunque hayamos preventDefault del mousedown. Solución multi-capa: (1) `editor.on('selectionUpdate', ...)` mantiene `stableSelectionRef` con la última selección. (2) Listener `mousedown` capture-phase a nivel `document` (button=2) snapshotea `stableSelectionRef → rightClickSnapshotRef` ANTES de que browser/PM puedan limpiar. (3) `handleContextMenu` setea `contextMenuSelection` desde el snapshot, con fallback al `editor.state.selection` live si por edge-case el snapshot está vacío y el live tiene contenido. (4) `EditorContextMenu` recibe `selectionSnapshot` prop y restaura la selección via `editor.chain().focus().setTextSelection({from, to}).run()` ANTES de cada comando (cut/copy/delete/comentar/link), garantizando que opera sobre el rango original.

**Bug crítico de Node import resuelto vía debug logs**: durante el desarrollo del fix de selección, los snapshots no se tomaban. User compartió console logs que mostraron `targetIsNode: false` para elementos `<p>`/`<span>`. Causa: `import { Node } from '@tiptap/core'` al inicio de ProjectEditor.jsx shadowea el DOM `Node` global. `e.target instanceof Node` siempre retornaba false → handlers hacían early return → snapshots nunca se tomaban. Fix: removido el check `instanceof Node`, confiando en `editorDom.contains(e.target)` (que ya valida que sea Node real). En otros lugares del archivo donde se necesita el check, usar `globalThis.Node` explícito.

**Fake selection extension** (`frontend/src/extensions/FakeSelection.js`): TipTap Extension con ProseMirror Plugin que mantiene un range en estado interno y aplica `Decoration.inline` con clase `.wb-fake-selection` (gris translúcido `rgba(160, 168, 180, 0.42)`). Comandos `setFakeSelection({from, to})` y `clearFakeSelection()` exposed. `handleContextMenu` los llama al abrir/cerrar el menú: la DOM selection nativa del browser se limpia al hacer right-click pero este overlay mantiene el feedback visual del rango sobre el que actuará el menú.

**Layout fix bells vs cards en page projects**: cuando hay `commentThreads` visibles en la página activa, `EditorPanel` agrega class `editorScrollAreaWithMargin` al scroll area que aplica `padding-right: 300px` con transición 200ms. Esto encoge el área de contenido → `editorPageRow` (con `margin: 0 auto`) se desplaza a la izquierda → activity bells (que viven inside `editorPageRow`) viajan con el canvas → no hay overlap con las cards (que están `position: absolute; right: 8px` del scrollAreaRef). `hasMarginComments` computado vía useMemo desde `commentThreads + commentLiveIds`.

**Hide cards + canvas min-width en viewports angostos**: `editorCanvas` ahora tiene `min-width: 500px` en `ProjectEditorSeoRules.module.css`; el canvas no se estruja debajo de eso aunque el viewport o el padding-right del scroll lo pidan. ResizeObserver mide `scrollAreaRef.width` en EditorPanel; si `< 900` (canvas min 500 + labels 42 + bells 42 + paddings + cards 300 ≈ 914), `cardsFitInViewport = false` → no se renderiza CommentMarginCards y NO se aplica `editorScrollAreaWithMargin`. Comportamiento Google Docs: ventana chica → comments desaparecen, canvas vuelve a centrar.

**Popover flotante para viewports angostos** (`frontend/src/components/editor/CommentInlinePopover.jsx`): cuando margin cards están ocultas (`!showMarginCards`) pero el usuario clickea un highlight `[data-comment-id]`, se renderiza un popover en `createPortal` con el thread completo. Position fixed; `useLayoutEffect` busca el span via `editor.view.dom.querySelector` y posiciona arriba o abajo del rango según `spaceBelow`/`spaceAbove`; si tampoco entra (raro), pin al borde con más espacio. Re-mide en scroll/resize y vía ResizeObserver del editor. Cierra con X (close button arriba derecha), click outside, Escape, o seleccionando otro thread (que reemplaza el contenido sin cerrar). Reusa `CommentEntry` y `ReplyComposer` exportados desde `CommentMarginCards.jsx` para mantener una sola implementación de la UI del thread.

**Auto-resolver comments huérfanos en backend** (PUT `/api/projects/:id/pages`): post-upsert query a `project_comments` por `parent_comment_id IS NULL AND deleted_at IS NULL AND resolved_at IS NULL`. Para cada page del payload, regex `data-comment-id\s*=\s*["']([^"']+)["']` extrae los IDs de comment presentes en el HTML nuevo → arma `idsByPage`. Cualquier root cuyo ID no esté en `idsByPage[c.page_id]` es huérfano → se actualiza con `resolved_at`, `resolved_by_user_id`, `status='resolved'`. Emite `logProjectActivity` con `event_type='comment_orphaned'`, `metadata: {commentId, pageId, pageName, anchorSnippet, originalBody, originalAuthor, originalActorId, reason: 'orphaned'}`. El feedback no se pierde: queda en DB resuelto y rastreable desde History.

**HistoryTabPanel sin gate de projectType** (extiende a document/faq): removido el bloqueo `projectType !== 'page'`. Para que el diff funcione: `buildDocumentActivityEvents` ahora incluye `sectionHtml` snapshot (HTML completo de la página) en cada event para projects `document` (lineales, sin sectionDivider). Dispatcher `saveProject` corregido: FAQ pasa por `buildSectionActivityEvents` (no `buildDocumentActivityEvents`) porque FAQ comparte el modelo sectionDivider con page → eventos granulares por FAQ section. Render del entry soporta `kind: 'edit' | 'orphan'`: orphans muestran sectionName (page name), snippet en cursiva entre comillas, body original del comentario; sin botón "Ver cambios" porque no aplica diff de HTML.

**Undo/redo globales aunque editor no tenga foco**: listener `keydown` a nivel `document` en ProjectEditor (montado on-mount). Si Cmd+Z (sin Shift) o Cmd+Shift+Z/Ctrl+Y, y `editor.isFocused === false`, y `document.activeElement` no es INPUT/TEXTAREA ni contenteditable externo (composers de comentarios viven en su propio scope), `preventDefault` y forward a `editor.commands.undo()`/`redo()`. Cuando el editor tiene foco, TipTap maneja vía su propio keymap y nuestro listener returns sin tocar.

**Browser shortcuts > editor shortcuts en colisiones**: la extensión oficial `@tiptap/extension-text-align` bindea `Mod-Shift-l/e/r/j` por default. `Mod-Shift-r` (hard refresh) y `Mod-Shift-j` (DevTools) chocan con el browser. Solución: `TextAlign.extend()` con `addKeyboardShortcuts` sobrescrito que retorna solo `Mod-Shift-l` y `Mod-Shift-e`. R y J ya no se bindean → browser tiene prioridad. Removida la extension custom `AlignShortcuts` (redundante con el override).

**CSS fix history card overflow**: `historyItemHeader` con `flex-wrap: wrap`; `historyItemTime` sin `white-space: nowrap` (cambio de la semántica original). Cuando el actor + fecha no caben en la misma línea que el sectionName, el time wrappea naturalmente a una segunda línea en vez de overflowear el card. Agregado `min-width: 0; word-break: break-word` a `historyItemSection` y `word-break: break-word` a `historyItemTime` como safety.

**Resend API key wired**: `RESEND_API_KEY` y `COMMENTS_EMAIL_FROM` configurados en `backend/.env` local + VPS. Smoke test contra `https://api.resend.com/emails` retornó `{"id": "..."}` (UUID), confirmando que la pipeline funciona.
