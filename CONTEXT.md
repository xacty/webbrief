...
- Dev keepalive: GitHub Actions workflow `.github/workflows/keepalive-supabase-dev.yml` (cron `0 12 */5 * *` + `workflow_dispatch`) hace un GET al REST API de Dev con la anon key para resetear el contador de auto-pause del free-tier. Secrets del repo `xacty/webbrief`: `SUPABASE_DEV_URL`, `SUPABASE_DEV_ANON_KEY`. Trigger manual: `gh workflow run keepalive-supabase-dev.yml`.
