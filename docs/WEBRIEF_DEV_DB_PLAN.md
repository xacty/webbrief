...
- **Free tier de Supabase Dev se pausa por baja actividad** (Supabase mide volumen de consultas en la ventana de 7 días, no tiempo desde la última request): mitigado desde 2026-07-30 con un workflow de GitHub Actions ([`.github/workflows/keepalive-supabase-dev.yml`](../.github/workflows/keepalive-supabase-dev.yml)). La cadencia original (1 GET cada 5 días) resultó insuficiente y Dev se pausó el 2026-08-31 pese a que todos los pings devolvían 200; desde el 2026-09-01 el workflow corre 4 veces al día con 3 consultas por run. Si aun así aparece pausada, hay que restaurarla desde el dashboard (Resume project) — con el proyecto pausado no hay DNS, así que ninguna query la reactiva.
