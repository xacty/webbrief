...
- Frontend fetch: `apiFetch(path, options)` en `frontend/src/lib/api.js:12` (agrega bearer de sesión Supabase; FormData soportado — no fuerza Content-Type; errores como `ApiError {status, payload}`). Descargas: `apiSubmitDownload(path, fields)` es form+iframe (POST fijo, fields como objeto plano, fire-and-forget: NO devuelve blob, NO expone headers ni status — hallazgo Task 10); para flujos que necesiten confirmar éxito usar fetch→blob (ver Task 14).
...
- `POST /assets/export` `{ ids, format?, width?, quality? }`: streaming ZIP con `archiver` replicando `export-bulk` (projects.js, grep `export-bulk`) pero resolviendo por `resolveCompanyAssetForExport`. **SIN auto-trash ni headers custom** (decisión post-Task 10: `apiSubmitDownload` usa form+iframe y no puede leer headers; la limpieza post-export la orquesta el CLIENTE llamando `bulk/trash` tras confirmar la descarga — ver Task 14). El endpoint solo exporta.
...
- [ ] **Step 1: LibraryExportModal** — formato segmented (WebP/PNG/JPG — patrón segmented control de DESIGN-SYSTEM §3), ancho máx (Input numérico, default 2560), calidad (Input numérico 1-100, default 80), toggle "Enviar a papelera tras exportar" (default ON, persistir preferencia en `localStorage['wb:library:trashAfterExport']`), texto de ayuda "Recuperables por 30 días". Submit (contrato decidido post-Task 10, NO usar `apiSubmitDownload` aquí): agregar a `libraryApi.js` un `downloadLibraryExport(companyId, { ids, format, width, quality })` que haga `fetch` POST con bearer de sesión (mismo patrón de token que `uploadLibraryAsset`), reciba el ZIP como blob, verifique `response.ok`, y dispare la descarga vía `URL.createObjectURL` + `<a download>`. SOLO si la descarga se confirmó OK y el toggle está ON: llamar `trashAssets(companyId, ids)` (sin `force`) y con su respuesta `{ trashed, kept }` mostrar el toast — si `kept.length`, "N enviadas a papelera · M conservadas (usadas en documentos)" con los `fileName` de kept. Si el export falla, no se trashea nada.
